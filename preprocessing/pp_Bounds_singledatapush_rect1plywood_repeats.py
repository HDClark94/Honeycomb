...
def extract_training_dataset(data):
	...
	avg_ft = np.mean(ft, 0)
	avg_ftx = avg_ft[0] #11 [10]
	avg_fty = avg_ft[1] #12 [11]
	avg_ftt = avg_ft[2] #13 [12]

	training_set = [tip_sx, tip_sy, tip_ex, tip_ey, object_sx, object_sy, object_stheta, object_ex, object_ey, object_etheta, avg_ftx, avg_fty, avg_ftt]
	return training_set

def json2trainingdata(filepath, shape_db, push_id):
	...
	with open(filepath) as data_file:    
		...
		vst_and_id = [float(v), float(s), float(t), push_id]
		...
		data_training_set.extend(vst_and_id)
...
def main(argv):
	...
	for json_filepath in filelist:
		...
		i = getfield_from_filename(json_filepath, 'i') #side number
		s = getfield_from_filename(json_filepath, 's') #contact point position on that side 0 to 1
		t = getfield_from_filename(json_filepath, 't') #contact angle in rad

		push_spec = (float(a), float(v), float(i), float(s), float(t))
		if (push_spec in push_specs):
			push_id = push_specs.index(push_spec)
		else:
			push_specs.append(push_spec)
			push_id = push_specs.index(push_spec)

		print a
		if (a == '0'): 
			all_training_data.append(json2trainingdata(json_filepath, shape_db, push_id))
			print 'len(all_training_data)', len(all_training_data)
			# except:
			# print json_filepath
			# pass
	
	outputfile= "%s/data_training_with_shapeBounds_andForce.json" % argv[1]
