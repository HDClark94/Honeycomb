...
class MixtureDensityNetwork(object):
  ...
  def _init_fc_layer(self, input, stddev = 0.1):
    ...
    Wh2 = tf.Variable(tf.random_normal([self._n_hidden, self._n_hidden], stddev=stddev, dtype=tf.float32))
    bh2 = tf.Variable(tf.random_normal([1, self._n_hidden], stddev=stddev, dtype=tf.float32))

    Wh3 = tf.Variable(tf.random_normal([self._n_hidden, self._n_hidden], stddev=stddev, dtype=tf.float32))
    bh3 = tf.Variable(tf.random_normal([1, self._n_hidden], stddev=stddev, dtype=tf.float32))

    Wh4 = tf.Variable(tf.random_normal([self._n_hidden, self._n_hidden], stddev=stddev, dtype=tf.float32))
    bh4 = tf.Variable(tf.random_normal([1, self._n_hidden], stddev=stddev, dtype=tf.float32))

    Wh5 = tf.Variable(tf.random_normal([self._n_hidden, self._n_hidden], stddev=stddev, dtype=tf.float32))
    bh5 = tf.Variable(tf.random_normal([1, self._n_hidden], stddev=stddev, dtype=tf.float32))
    ...
    hidden_layer2 = tf.nn.tanh(tf.matmul(hidden_layer1, tf.multiply(tf.clip_by_norm(Wh2, self._max_weight_mag), self._weight_multiplier)) + bh2)
    hidden_layer2 = tf.nn.dropout(hidden_layer2, self._dropout_prob)
    hidden_layer3 = tf.nn.tanh(tf.matmul(hidden_layer2, tf.multiply(tf.clip_by_norm(Wh3, self._max_weight_mag), self._weight_multiplier)) + bh3)
    hidden_layer3 = tf.nn.dropout(hidden_layer3, self._dropout_prob)
    hidden_layer4 = tf.nn.tanh(tf.matmul(hidden_layer3, tf.multiply(tf.clip_by_norm(Wh4, self._max_weight_mag), self._weight_multiplier)) + bh4)
    hidden_layer4 = tf.nn.dropout(hidden_layer4, self._dropout_prob)
    hidden_layer5 = tf.nn.tanh(tf.matmul(hidden_layer4, tf.multiply(tf.clip_by_norm(Wh5, self._max_weight_mag), self._weight_multiplier)) + bh5)
    hidden_layer5 = tf.nn.dropout(hidden_layer5, self._dropout_prob)
    output_fc = tf.matmul(hidden_layer5, tf.multiply(tf.clip_by_norm(Wo, self._max_weight_mag), self._weight_multiplier)) + bo

    # weightSum calculated for weight size regularization
    weightSquaredSum = tf.reduce_sum(tf.square(Wh1)) + tf.reduce_sum(tf.square(Wh2)) + tf.reduce_sum(tf.square(Wh3)) + tf.reduce_sum(tf.square(Wh4)) + tf.reduce_sum(tf.square(Wh5)) + tf.reduce_sum(tf.square(Wo)) + tf.reduce_sum(tf.square(bh1)) + tf.reduce_sum(tf.square(bh2)) + tf.reduce_sum(tf.square(bh3)) + tf.reduce_sum(tf.square(bh4)) + tf.reduce_sum(tf.square(bh5)) + tf.reduce_sum(tf.square(bo))
    #----------------------
