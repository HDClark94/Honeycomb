...
class MixtureDensityNetwork(object):
  ...
  def _init_fc_layer(self, input, stddev = 0.1):
    ...
    output_fc = tf.matmul(hidden_layer4, tf.multiply(tf.clip_by_norm(Wo, self._max_weight_mag), self._weight_multiplier)) + bo

    # weightSum calculated for weight size regularization
    weightSquaredSum = tf.reduce_sum(tf.square(Wh1)) + tf.reduce_sum(tf.square(Wh2)) + tf.reduce_sum(tf.square(Wh3)) + tf.reduce_sum(tf.square(Wh4)) + tf.reduce_sum(tf.square(Wo)) + tf.reduce_sum(tf.square(bh1)) + tf.reduce_sum(tf.square(bh2)) + tf.reduce_sum(tf.square(bh3)) + tf.reduce_sum(tf.square(bh4)) + tf.reduce_sum(tf.square(bo))
    #----------------------
